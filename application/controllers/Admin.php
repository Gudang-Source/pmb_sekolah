...
class Admin extends CI_controller
{
 ...
 if ($data->num_rows() > 0) {
  ...
  if ($data->row()->konfirmasi == "N") {
    buat_alert('Data Belum Bisa Di Cetak Pastikan Anggota Sudah Di Terima');
  }else{
    $x['dat']=$this->db->get_where('rn_daftar',array('id_pendaftaran'=>$id))->result_array();
    $this->load->view('admin/cetak_data_pmb',$x);
  }
}else{
  ...
  $x['judul'] = "Data Penerimaan Anggota Pelatihan.";
...
public function cetak_pendaftar($action='',$dari='',$sampai='',$tahun_akademik='')
{
  if (empty($tahun_akademik)) {
   ...
   if($action == 'excel'){
     $x['judul'] ='Rekap Hasil Kelulusan Peserta Pendaftar Pelatihan.';
...
/*batas akhir kodingan*/
  ...
  $x=array('judul'=>'Sys Information Management | E-research ',
  ...
  if (isset($_POST['kirim'])) {
    ...
    if($this->form_validation->run() == FALSE){
     ...
     if ($this->upload->do_upload('admin')) {
      ...
      $this->session->set_flashdata($this->upload->display_error('pesan','<div class="callout callout-info">','</div>'));
      redirect(base_url('admin/profil'));
    }
  }     
}else{
...
if ($action == 'add') {
  ...
  if(isset($_POST['kirim'])){
   ...
   $cek=$this->db->insert('jurusan',$data);
   if ($cek) {
    $this->session->set_flashdata('pesan','<div class="callout callout-info">Insert data jurusan berhasil.</div>');
    redirect(base_url('admin/data_jurusan'));
  }else{
    $this->session->set_flashdata('pesan','<div class="callout callout-info">Gagal data jurusan.</div>');
    redirect(base_url('admin/data_jurusan'));
  }
